...
test('planEventReminders: gera lembrete minutos antes', () => {
  ...
  const out = s.Notifications.planEventReminders([ev], '2026-08-20', { daysAhead: 10 })
...
test('planEventReminders: ignora evento sem horário ou fora do horizonte', () => {
  const s = fresh()
  const out = s.Notifications.planEventReminders([
...
test('planBillReminders: 5 dias antes, 1 dia antes e no dia', () => {
  ...
  const out = s.Notifications.planBillReminders([bill], '2026-08-15', [5, 3, 2, 1])
  // 5 dias antes: 15/08 09:00 (2026-08-20 - 5 dias)
  assert.strictEqual(out.length, 1)
  assert.strictEqual(out[0].timestamp, new Date('2026-08-15T09:00:00').getTime())
  const out2 = s.Notifications.planBillReminders([bill], '2026-08-19', [5, 3, 2, 1])
...
test('planBillReminders: título do dia do vencimento é crítico', () => {
  ...
  const out = s.Notifications.planBillReminders([bill], '2026-08-20', [5, 3, 2, 1])
  assert.strictEqual(out.length, 1)
...
test('morningSummary: conta atividades do dia', () => {
  const s = fresh()
  const out = s.Notifications.morningSummary(
...
test('planAll: respeita prefs desabilitadas', () => {
  ...
  const out = s.Notifications.planAll([], [], [], settings, '2026-08-20')
  assert.strictEqual(out.length, 0)
})
...
test('planAll: usa daysBefore da config', () => {
  ...
  const out = s.Notifications.planAll([], [], [bill], settings, '2026-08-18')
  assert.strictEqual(out.length, 1)
